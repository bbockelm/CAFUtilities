...
def define_the_build(self, dist, system_name, patch_x = ''):
 ...
 # dist.py_modules = ['CRABServer.__init__']
  pkgs = find_packages('src/python')
  toBuild = []
  for pkg in pkgs: 
      if len(pkg.split('.')) > 1: toMatch = pkg.split('.')[0]
      else: toMatch = pkg  
      if toMatch in system['pkg']['python']:
          toBuild.append(pkg) 
  dist.packages = toBuild#system['pkg']['python']
  #dist.data_files = [('%sbin' % patch_x, binsrc)]
  dist.data_files = []
  dist.scripts =  system['data']['python']
...
class BuildCommand(Command):
  ...
  description = \
    ...
    "\t\t   at the moment are 'CAFTaskWorker' and 'CAFUtilities'. Use with --force \n" + \
  ...
  def generate_docs(self):
    if not self.skip_docs:
     # os.environ["PYTHONPATH"] = "%s/../WMCore/src/python/:%s" % (os.getcwd(), os.environ["PYTHONPATH"])
      ...
      spawn(['make', '-C', 'doc', 'html', 'PROJECT=%s' % 'crabutilities' ])
